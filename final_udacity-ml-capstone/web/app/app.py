...
class App:
    ...
    def predict(self, img_path):
        ...
        if is_dog or is_human:
            ...
            dogs_breed = self.get_dogs_breed(image)
    ...
    def is_human(self, img_path):
        ...
        '''
        face_cascade = cv2.CascadeClassifier(os.path.abspath('./app/haarcascades/haarcascade_frontalface_alt.xml'))
    ...
    def build_message(self, is_dog, is_human, dogs_breed):
        ...
        '''
        if is_dog:
            return "Who is a good boy/girl?!?! Yes you are! <br/> You are a beautiful {breed}".format(breed=dogs_breed)
        elif is_human:
            return "Oh! Hello human being, this is a dog's breed classifer!<br/>But looking at you again ... you really look like a {breed}".format(breed=dogs_breed)
        else:
            return "Wait? It doesn't look like a dog or a human face, please, try again!"
    ...
    def get_model(self):
        ...
        model.classifier[6] = torch.nn.Linear(in_features=n_inputs, out_features=len(self.classes))
        # load trained model from disk
        model.load_state_dict(torch.load(os.path.abspath('./app/model/model.pt'), map_location=torch.device('cpu')))
